 ...
 *
 * Note: For API sources like World Bank WGI that don't support delta queries,
 * we re-fetch and compare item IDs. The Graph API handles idempotent upserts,
 * so re-ingesting unchanged items is safe (if wasteful).
 * CSV-based sources (CPI, Freedom House) are only included in full crawls.
 */
async function incrementalCrawl(
  ...
  // Fetch current API data (World Bank WGI)
  try {
    const wgiRecords = await fetchWorldBankWGI();
    allRecords.push(...wgiRecords);
  } catch (error: unknown) {
    context.error(
      `World Bank WGI incremental fetch failed: ${error instanceof Error ? error.message : error}`
    );
  }
...
app.timer("incrementalCrawl", {
  schedule: "0 0 6 * * 1", // Weekly, Monday at 6 AM UTC
