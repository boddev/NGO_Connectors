...
import { fetchFaostatData } from "../dataSources/faostat.js";
...
import type { AgricultureRecord } from "../dataSources/types.js";

const crawlQueueOutput = output.storageQueue({
  queueName: "ngo-agriculture-crawl-requests",
...
async function executeFullCrawl(
  ...
  const allRecords: AgricultureRecord[] = [];

  // Source 1: World Bank
  context.log("Fetching World Bank agriculture data...");
  try {
  ...
  context.log("Fetching Our World in Data agriculture datasets...");
  try {
  ...
  // Source 3: FAOSTAT (may be unavailable — graceful fallback)
  context.log("Fetching FAOSTAT crop production data...");
  try {
    const faoRecords = await fetchFaostatData();
    context.log(`FAOSTAT: ${faoRecords.length} records fetched`);
    allRecords.push(...faoRecords);
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : String(error);
    context.warn?.(`FAOSTAT fetch skipped: ${msg}`);
  }
  ...
  state.sources["faostat"] = {
    lastSync: new Date().toISOString(),
    itemCount: allRecords.filter((r) => r.sourceOrganization === "FAO").length,
    lastChecksum: "",
    enabled: state.sources["faostat"]?.enabled ?? true,
  };
