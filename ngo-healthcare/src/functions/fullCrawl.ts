...
const crawlQueueOutput = output.storageQueue({
  queueName: "ngo-healthcare-crawl-requests",
...
async function executeFullCrawl(
  ...
  // Source 1: World Bank
  context.log("Fetching World Bank health data...");
  try {
  ...
  // Source 2: WHO Global Health Observatory
  context.log("Fetching WHO GHO data...");
  try {
    const whoRecords = await fetchWhoData();
    context.log(`WHO GHO: ${whoRecords.length} records fetched`);
    allRecords.push(...whoRecords);
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : String(error);
    context.error(`WHO GHO fetch failed: ${msg}`);
  }
  ...
  state.sources["who"] = {
    ...
    itemCount: allRecords.filter(
      (r) => r.sourceOrganization === "World Health Organization"
    ).length,
    lastChecksum: "",
    enabled: state.sources["who"]?.enabled ?? true,
  };
