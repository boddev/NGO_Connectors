...
import type { GenderRecord } from "../dataSources/types.js";

const crawlQueueOutput = output.storageQueue({
  queueName: "ngo-gender-crawl-requests",
...
async function executeFullCrawl(
  ...
  const allRecords: GenderRecord[] = [];

  // Source 1: World Bank
  context.log("Fetching World Bank gender data...");
  try {
  ...
  context.log("Fetching Our World in Data gender indicators...");
  try {
