...
import type { ChildProtectionRecord } from "../dataSources/types.js";

const crawlQueueOutput = output.storageQueue({
  queueName: "ngo-childprotection-crawl-requests",
...
async function executeFullCrawl(
  ...
  const allRecords: ChildProtectionRecord[] = [];

  // Source 1: World Bank
  context.log("Fetching World Bank child protection data...");
  try {
  ...
  // Source 2: Our World in Data (child mortality, labor, stunting)
