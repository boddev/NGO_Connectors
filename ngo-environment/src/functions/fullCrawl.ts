...
import { parseEmDatCsv, getEmDatCsvPath } from "../dataSources/emdat.js";
...
import type { EnvironmentRecord } from "../dataSources/types.js";

const crawlQueueOutput = output.storageQueue({
  queueName: "ngo-environment-crawl-requests",
...
async function executeFullCrawl(
  ...
  const allRecords: EnvironmentRecord[] = [];

  // Source 1: World Bank
  context.log("Fetching World Bank climate data...");
  try {
  ...
  // Source 3: EM-DAT (from local CSV if available)
  const emdatPath = getEmDatCsvPath();
  context.log(`Checking for EM-DAT CSV at: ${emdatPath}`);
  try {
    const emdatRecords = parseEmDatCsv(emdatPath);
    context.log(`EM-DAT: ${emdatRecords.length} records parsed`);
    allRecords.push(...emdatRecords);
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : String(error);
    context.warn?.(`EM-DAT parse skipped: ${msg}`);
  }
  ...
  state.sources["emdat"] = {
    lastSync: new Date().toISOString(),
    itemCount: allRecords.filter((r) => r.sourceOrganization === "CRED/EM-DAT")
      .length,
    lastChecksum: "",
    enabled: state.sources["emdat"]?.enabled ?? false,
  };
