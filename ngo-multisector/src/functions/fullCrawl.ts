...
import type { MultiSectorRecord } from "../dataSources/types.js";

const crawlQueueOutput = output.storageQueue({
  queueName: "ngo-multisector-crawl-requests",
...
async function executeFullCrawl(
  ...
  const allRecords: MultiSectorRecord[] = [];

  // Source 1: World Bank cross-cutting indicators
  context.log("Fetching World Bank cross-cutting data...");
  try {
  ...
  // Source 2: Our World in Data cross-cutting CSVs
