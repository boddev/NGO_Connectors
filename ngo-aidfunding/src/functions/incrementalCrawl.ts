...
import { fetchIatiPublishers } from "../dataSources/iatiRegistry.js";
 ...
 *
 * Note: For API sources like World Bank and IATI that don't support
...
async function incrementalCrawl(
  ...
  try {
    ...
    context.error(
      `World Bank ODA incremental fetch failed: ${error instanceof Error ? error.message : error}`
    );
  }

  try {
    const iatiRecords = await fetchIatiPublishers();
    allRecords.push(...iatiRecords);
  } catch (error: unknown) {
    context.error(
      `IATI Registry incremental fetch failed: ${error instanceof Error ? error.message : error}`
    );
  }
