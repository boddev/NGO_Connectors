 ...
 *
 * Note: ProPublica API doesn't support delta queries, so we re-fetch and
 * compare item IDs. The Graph API handles idempotent upserts, so
 * re-ingesting unchanged items is safe (if wasteful).
...
async function incrementalCrawl(
  ...
  // Fetch current data from ProPublica
  try {
    const ppRecords = await fetchProPublicaData();
    allRecords.push(...ppRecords);
  } catch (error: unknown) {
    context.error(
      `ProPublica incremental fetch failed: ${error instanceof Error ? error.message : error}`
    );
  }
...
app.timer("incrementalCrawl", {
  schedule: "0 0 6 * * 1", // Weekly on Monday at 6 AM UTC
