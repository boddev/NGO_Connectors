...
import type { WashRecord } from "../dataSources/types.js";

const crawlQueueOutput = output.storageQueue({
  queueName: "ngo-wash-crawl-requests",
...
async function executeFullCrawl(
  ...
  context.log("Starting full WASH crawl...");
  const allRecords: WashRecord[] = [];

  // Source 1: World Bank WASH indicators
  context.log("Fetching World Bank WASH data...");
  try {
  ...
  // Source 2: Our World in Data (WHO WASH / JMP)
  context.log("Fetching Our World in Data WASH...");
  try {
...
// Timer-triggered full crawl (1st of month, 2 AM UTC)
async function fullCrawlTimer(
  timer: Timer,
  context: InvocationContext
): Promise<void> {
  context.log("Full WASH crawl timer triggered");
...
async function onDemandCrawl(
  ...
  context.log("On-demand WASH crawl triggered via HTTP");
