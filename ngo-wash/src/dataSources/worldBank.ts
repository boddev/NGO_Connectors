...
// World Bank WASH indicator codes with metadata
...
/**
 * Fetch all WASH indicators from the World Bank API.
 * Paginates through all countries for each indicator code.
 */
export async function fetchWorldBankData(): Promise<WashRecord[]> {
    ...
    while (page <= totalPages) {
      ...
      for (const dp of data) {
        ...
        records.push({
          ...
          indicatorValue: `${dp.value.toFixed(1)}%`,
          ...
          coveragePercent: dp.value,
          waterSourceType: "",
          urbanRural: "Total",
          tags: [ind.washService, ind.serviceLevel, "World Bank", ind.code],
